...
function MyPosts() {
    ...
    const [editData, setEditData] = useState({ id: "", title: "", message: "", postImg: undefined });
    ...
    return (<div>
            ...
            {localStoragePosts.length > 0 ? (<ul>
                    {localStoragePosts.map((post, index) => (<li key={`local-${index}`} className="my-post-container p2">
                            <div>
                                {editMode && post._id === editData.id ? (<div className="edit-form">
                                        <label className="input-txt">Title:</label>
                                        <input type="text" value={editData.title} onChange={(e) => setEditData({ ...editData, title: e.target.value })} placeholder="Enter title"/>
                                        <label className="input-txt">Message:</label>
                                        <textarea value={editData.message} onChange={(e) => setEditData({ ...editData, message: e.target.value })} placeholder="Enter message" rows={4}/>
                                        <label className="input-txt">Post Image:</label>
                                        <input type="file" accept="image/*" onChange={onImgSelected}/>
                ...
                </ul>) : (<p className='noPost'>No Created Posts yet!</p>)}
        </div>);
}
